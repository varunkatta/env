...
| 2026-09-04 | Confirmed | Agents should proactively suggest cautious operating-model directives when recurring requests or interaction/project history support a useful generalization. | Surface the observation, proposed directive, and expected cross-project benefit; ask whether it should become durable memory. Do not infer a standing rule from one request or persist it without explicit approval. |
...
| 2026-09-05 | Confirmed | Code commits require the project/language formatter and linter to pass first. | Discover the repository-configured tools, run both formatter and linter for the changed language plus required quality gates, fix findings, and record the exact evidence in the handoff. |
| 2026-09-05 | Confirmed | Diagnose a blocked PR from repository evidence before claiming an approval is required. | Inspect mergeability, active rulesets, checks, and unresolved review threads; resolve or repair thread blockers immediately, and name a human approver only when a rule actually requires one. |
