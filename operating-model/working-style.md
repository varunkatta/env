...
| Authorization momentum | An external authorization boundary must not turn into passive monitoring. | Ask once with the exact destination, action, and payload class; continue other safe leased work; execute the queued action in the first writable turn after authorization; do not repeat unchanged status notifications. |
| Merge-blocker diagnosis | A blocked PR must be investigated to the exact rule, check, or review thread before it is described as needing approval. | Inspect mergeability metadata, active rulesets, checks, and unresolved threads; resolve incorrect/stale threads with canonical evidence and repair valid ones. Name an external approver only when the active rule genuinely requires one. |
| Execution release | Detecting that a prerequisite is satisfied is not progress unless it releases the next safe action. | Maintain an explicit next-action queue for each active dependency; when a merge/check/review satisfies its condition, dispatch the queued leased task in the first writable turn and report the dispatch, not merely the observation. |
| Code quality gate | Every code change must satisfy the repository's language- and project-specific formatter and linter before commit. | Discover the configured tools from the project, run both on changed code (and the required project gate when practical), fix findings rather than merely reporting them, and record the commands/results in the handoff. |
...
| Cost-aware delegation | The orchestrating session is the most expensive place for work to happen, so it does not do bounded work itself; but the model that does the work must match the judgment the work needs, because a cheaper model's error in design work costs more than the saving. | Before anything larger than a one-line answer, pick the tier by this rule and state the choice in the brief and the report; ask the owner only when the tier is unclear. **Top-tier model** (the orchestrator's own tier, run as a subagent) for judgment-heavy work whose errors propagate into a design, a ruling or cross-repository alignment: design and interface specs, drafting ruling batches, alignment reads against an upstream repository, critiques, measurement design, ADR review. **Mid-tier model** only for straightforward work with a clear spec whose mistakes surface in tests or a diff: inventories and lookups against a defined question, bounded code changes from a ratified brief, applying ratified rulings to documents, harness plumbing. **Smallest model** for mechanical work where the orchestrator supplies the exact text or command: exact-string edits, commits and PRs, review-thread fixes with the wording given, merges of the main branch, link checks, bulk renames. Whatever the tier, the orchestrator writes the brief and verifies every output itself before it reaches the owner. Run work through a subagent with an explicit model or a written handoff to another session, not the orchestrating session's own tool calls. See `execution-and-handoffs.md` → Mechanical batch delegation. |
